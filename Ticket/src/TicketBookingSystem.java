...
public class TicketBookingSystem extends JFrame {
	...
	private final Color peonyAccent = new Color(142, 92, 98);
	...
	public TicketBookingSystem() {
		...
		spNumOfTicket = new JSpinner( new SpinnerNumberModel (0, 0, 48, 1)); //(INITIAL VALUE, MININMUM VALUE, MAXIMUM VALUE, STEPS) 16 x  3 = 48
		...
		pnlEvent1 = new JPanel(); //CONTAINER OF SEAT 1 (FHUKERAT)
		...
		model1 = new DefaultTableModel(
			...
			new String[] {
				"Name", "Event", "Mode", "Seat", "Price"
			}
		);
		
		//DUMMY DATA
		model1.addRow(new Object[]{"Jherwel", "Fhukerat", "Semi-VIP", "Seat 6", 200});
		model1.addRow(new Object[]{"Jirah", "Fhukerat", "Semi-VIP", "Seat 9", 200});
		model1.addRow(new Object[]{"Faith", "Fhukerat", "Regular", "Seat 13", 100});
		...
		JCheckBox chckbxFhukerat = new JCheckBox("Fhukerat");
		chckbxFhukerat.setOpaque(false);
		chckbxFhukerat.setForeground(peonyText);
		chckbxFhukerat.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				if(chckbxFhukerat.isSelected()) {
					tabbedPane.addTab("Fhukerat", pnlEvent1);
				}else {
					tabbedPane.remove(pnlEvent1);
				}
			}
		});
		chckbxFhukerat.setBounds(6, 92, 92, 20);
		
		JCheckBox chckbxAlfyBara = new JCheckBox("Alfy Bara");
		chckbxAlfyBara.setOpaque(false);
		chckbxAlfyBara.setForeground(peonyText);
		chckbxAlfyBara.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				if(chckbxAlfyBara.isSelected()) {
					tabbedPane.addTab("Alfy", pnlEvent2);
				}else {
					tabbedPane.remove(pnlEvent2);
				}
			}
		});
		chckbxAlfyBara.setBounds(152, 92, 92, 20);
		
		JCheckBox chckbxTaylorSwift = new JCheckBox("Taylor Swift");
		chckbxTaylorSwift.setOpaque(false);
		chckbxTaylorSwift.setForeground(peonyText);
		chckbxTaylorSwift.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(chckbxTaylorSwift.isSelected()) {
					tabbedPane.addTab("Taylor", pnlEvent3);
				}else {
					tabbedPane.remove(pnlEvent3);
				}
			}
		});
		chckbxTaylorSwift.setBounds(285, 92, 92, 20);
				...
				public void actionPerformed(ActionEvent e) {
					name = txtName.getText();
					 ...
					 if(name.equals("admin") && password.equals("123")) {
						 ...
						 JOptionPane.showMessageDialog(null, "Incorrect usename or password", "ERROR MESSAGE", JOptionPane.ERROR_MESSAGE);
					 }
				}
			});
			...
			JLabel lblTagline = new JLabel(
			    "Bloom into every show with Peony tickets.",
			...
			btnClear.addActionListener(new ActionListener() {
				...
				btnBook.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						for(int i = 0; i < 16; i++) {
							...
							// FHUKERAT
							if(event1[i] == true) {
								spinnerValue--; //UPDATES THE VALUE OF THE SPINNER EVERY SUCCESSFUL TRANSACTION
								String mode;
								String seat = "";
								double price = 0;
								...
								model.addRow( new Object[] {
										"Fhukerat", mode, seat, price
								});
								model1.addRow( new Object[] {
										name, "Fhukerat", mode, seat, price
								});
							...
							if(event2[i] == true) {
								...
								model.addRow( new Object[] {
										"Alfy Bara", mode, seat, price
								});
								
								model1.addRow( new Object[] {
										name, "Alfy Bara", mode, seat, price
								});
								event2[i] = false;
							}
							...
							if(event3[i] == true) {
								...
								model.addRow( new Object[] {
										"Taylor Swift", mode, seat, price
								});
								
								model1.addRow( new Object[] {
										name, "Taylor Swift", mode, seat, price
								});
								event3[i] = false;
							}
						}
						
						spNumOfTicket.setValue(spinnerValue); //THIS REWRITE THE VALUE OF THE SPINNER
				...
				centerSeats(pnlSeatSelection, btnBack1, spNumOfTicket, lblNumOfTicket,
						chckbxFhukerat, chckbxAlfyBara, chckbxTaylorSwift, tabbedPane,
						scrollPane, pnlButtons, btnClear, btnBook, btnTransac);
				centerTransaction(pnlTransaction, btnBack2, lblTransac, spTransaction);
			
	}

	private void centerLogin(JPanel pnl, JLabel banner, JLabel lblTagline, JTextField txtUser,
	        JPasswordField txtPass, JCheckBox showPass, JButton btnLogin) {
	    pnl.removeAll();
	    ...
	    GridBagConstraints gbc0 = makeGbc(0, 0, GridBagConstraints.NONE, GridBagConstraints.CENTER, new Insets(8, 12, 8, 12));
	    pnlForm.add(banner, gbc0);

	    GridBagConstraints gbc1 = makeGbc(0, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER, new Insets(8, 12, 8, 12));
	    pnlForm.add(lblTagline, gbc1);

	    txtUser.setPreferredSize(new Dimension(405, 31));
	    GridBagConstraints gbc2 = makeGbc(0, 2, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER, new Insets(8, 12, 8, 12));
	    pnlForm.add(txtUser, gbc2);

	    txtPass.setPreferredSize(new Dimension(405, 31));
	    GridBagConstraints gbc3 = makeGbc(0, 3, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER, new Insets(8, 12, 8, 12));
	    pnlForm.add(txtPass, gbc3);

	    GridBagConstraints gbc4 = makeGbc(0, 4, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST, new Insets(8, 12, 8, 12));
	    pnlForm.add(showPass, gbc4);

	    GridBagConstraints gbc5 = makeGbc(0, 5, GridBagConstraints.NONE, GridBagConstraints.CENTER, new Insets(8, 12, 8, 12));
	...
	private void centerMenu(JPanel pnl, JMenuBar menuBar, JButton btnBookSeat,
			JButton btnTransac) {
		pnl.removeAll();
		pnl.setBackground(peonyBg);
		...
		GridBagConstraints gbc0 = makeGbc(0, 0, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER, new Insets(12, 32, 12, 32));
		pnlChoices.add(lblTitle, gbc0);

		GridBagConstraints gbc1 = makeGbc(0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER, new Insets(12, 32, 12, 32));
		pnlChoices.add(lblStatus, gbc1);

		GridBagConstraints gbc2 = makeGbc(0, 2, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER, new Insets(12, 32, 12, 32));
		pnlChoices.add(btnBookSeat, gbc2);

		GridBagConstraints gbc3 = makeGbc(0, 3, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER, new Insets(12, 32, 12, 32));
		pnlChoices.add(btnTransac, gbc3);

		pnl.add(pnlChoices, BorderLayout.CENTER);
	}
	
	private void centerSeats(JPanel pnl, JButton btnBack, JSpinner spinner,
			JLabel lblTicket, JCheckBox chkFhukerat, JCheckBox chkAlfy, JCheckBox chkTaylor,
			JTabbedPane tabs, JScrollPane tableScroll, JPanel pnlButtons,
		...
		pnlSelect.add(spinner, makeGbc(0, 0, GridBagConstraints.NONE, GridBagConstraints.CENTER,
				new Insets(4, 8, 4, 8)));
		pnlSelect.add(lblTicket, makeGbc(1, 0, GridBagConstraints.NONE, GridBagConstraints.CENTER,
				new Insets(4, 8, 4, 8)));
		pnlSelect.add(chkFhukerat, makeGbc(0, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER,
				new Insets(4, 8, 4, 8)));
		pnlSelect.add(chkAlfy, makeGbc(1, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER,
				new Insets(4, 8, 4, 8)));
		pnlSelect.add(chkTaylor, makeGbc(2, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER,
				new Insets(4, 8, 4, 8)));
	...
	private void centerTransaction(JPanel pnl, JButton btnBack, JLabel lblTitle,
	                ...
	                buttonsEvent1[currentIndex].addActionListener(e -> {
	                    ...
	                    if (!event1[currentIndex]) {
	                        ...
	                        if (totalSelectedSeats < maxTickets) {
	                            ...
	                            JOptionPane.showMessageDialog(null,
	                                    "You can only select " + maxTickets + " seats.",
	                                    "INFORMATION MESSAGE",
	                                    JOptionPane.INFORMATION_MESSAGE);
	                        }
